...
def get_contig_json(contig_obj, bin_size):
    # You need to decrement the lower bound because base pairs are 1-indexed.
    # Otherwise, subject_start = 100, subject_end = 100, bin_size = 10 will give you contig_bin_index = 10.
    # And bin_index can only be 0 to 9.
    contig_range = decrement_lower_bound((contig_obj["subject_start"], contig_obj["subject_end"]))
    contig_midpoint = (contig_range[0] + contig_range[1]) / 2
    contig_bin_index = safe_floor(contig_midpoint / bin_size)

    return [
        # Read count
        contig_obj["num_reads"],
        # Alignment range
        contig_obj["subject_start"],
        contig_obj["subject_end"],
        # Alignment length. Can be different from alignment range.
        contig_obj["alignment_length"],
        # Percent identity
        contig_obj["percent_id"],
        # Number of mismatches
        contig_obj["num_mismatches"],
        # Number of gaps
        contig_obj["num_gaps"],
        # Bin index of midpoint. Used to display contig if contig is too small.
        contig_bin_index
    ]

def generate_contig_json(accession_data, accession_id, contigs_map, num_bins):
    bin_size = accession_data["total_length"] / num_bins
    contigs = []
    for contig_name in accession_data["contigs"]:
        if contig_name in contigs_map:
            ...
            contigs.append(get_contig_json(contig_obj, bin_size))
        else:
...
def get_hit_group_json(contig_objs, read_objs, bin_size):
    ...
    return [
        ...
        # Percent identity
        format_number(avg_field("percent_id")),
        # Number of mismatches
...
def select_best_accessions_per_taxon(taxons_to_accessions, accession_data, contigs_map, reads_map, num_accessions_per_taxon):
    def get_score(accession_id):
        ...
        read_lengths = [reads_map[read_name]["alignment_length"] for read_name in accession_obj["reads"]]
        ...
        # Scale down the read_lengths so that contigs matter much more.
        # This is only a tiebreaker if there are no contigs.
        read_depth = sum(read_lengths) / accession_obj["total_length"]

        return max_contig_length + total_contig_length + read_depth
    ...
    for taxon, accessions in taxons_to_accessions.items():
        ...
        # Take ALL accessions with one or more contigs. These accessions should have score > 1.
